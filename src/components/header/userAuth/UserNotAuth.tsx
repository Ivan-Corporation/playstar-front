...
function UserNotAuth({changeAuth}: UserNotAuthProps): JSX.Element {
    ...
    const [ModalReset, reset_open, reset_close, reset_isOpen] = useModal('root', {
        preventScroll: true,
        closeOnOverlayClick: true
      });


    const [ModalRegistrationStepTwo, registration_two_open, registration_two_close, registration_two_isOpen] = useModal('root', {
        preventScroll: true,
        closeOnOverlayClick: true
      });
      ...
      function changeToSecondRegistrationStep() {
        registration_close()
        registration_two_open()
      }


      function registrationSuccess() {
        registration_two_close()
        changeAuth(true)
      }
    ...
    return (
      ...
      <ModalLogin>
        <div className='modal__login'>
          ...
          <div className="modal__form">
                <input className="modal__form_input" placeholder="Никнейм или номер телефона" type='text' required/>
            </div>
          <div className="modal__form">
                <input className="modal__form_input" placeholder="Пароль" type="password" required/>
            </div>
            <a onClick={changeToResetPassword} className='modal__reset-pass'>Забыли пароль?</a>
            
            <div className='modal_buttons'>
          <button className='modal__button_blue' onClick={login_close}>ВОЙТИ</button>
          <button className='modal__button_blue-border' onClick={changeToRegistration}>РЕГИСТРАЦИЯ</button>
          </div>
        </div>
      </ModalLogin>
      ...
      <ModalReset>
        ...
        <img onClick={reset_close} className='modal__soc-icons-close' src={close_modal} alt='so-icons'/>

          <h1 className='modal__title' style={{marginBottom:'29px'}}>Восстановить пароль</h1>
         
          <div className="modal__form">
                <input className="modal__form_input" placeholder="Ваш номер телефона" type='text' required/>
            </div>
          <div className="modal__form">
          ...
          <button className='modal__button_blue' onClick={reset_close}>ОТПРАВИТЬ</button>
          </div>
        </div>
      </ModalReset>
      ...
      <ModalRegistration>
        ...
        <img onClick={registration_close} className='modal__soc-icons-close' src={close_modal} alt='so-icons'/>
          ...
          <div className="modal__form">
                <input className="modal__form_input" placeholder="Никнейм" type='text' required/>
            </div>
          <div className="modal__form">
                <input className="modal__form_input" placeholder="Почта" type="text" required/>
            </div>

            <p className='modal__email-check'>Мы отправили на почту ссылку, перейдите по ней - если письмо не пришло, проверьте паапку СПАМ, а также корректность введенного E-mail</p>
             
            <div className="modal__form">
                <input className="modal__form_input" placeholder="Пароль" type='password' required/>
            </div>
          <div className="modal__form">
                <input className="modal__form_input" placeholder="Повторите пароль" type="password" required/>
            </div>
           
            <div className='modal_buttons'>
          <button style={{marginTop:'18px', width:'50%', height:'45px', borderRadius:'5px'}} className='modal__button_blue' onClick={changeToSecondRegistrationStep}>ЗАРЕГИСТРИРОВАТЬСЯ</button>
          </div>
        </div>
      </ModalRegistration>
      ...
      <ModalRegistrationStepTwo>
        <div className='modal__registration-two'>
        <img onClick={registration_two_close} className='modal__soc-icons-close' src={close_modal} alt='so-icons'/>
          ...
          <p className='modal__email-check'>Завершите регистрацию, введя свой номер телефона для связи*</p>

          <div className="modal__form">
                <input className="modal__form_input" placeholder="Никнейм" type='text' required/>
            </div>
            <p className='modal__email-check'>Мы позвоним, введите 4  последнии цифры номера , отвечать на звонок не надо</p>
          ...
          <div className="modal__form">
            ...
            <div className='modal_buttons'>
          <button style={{marginTop:'18px', width:'50%', height:'45px', borderRadius:'5px'}} className='modal__button_blue' onClick={registrationSuccess}>ЗАРЕГИСТРИРОВАТЬСЯ</button>
          </div>
        </div>
      </ModalRegistrationStepTwo>
